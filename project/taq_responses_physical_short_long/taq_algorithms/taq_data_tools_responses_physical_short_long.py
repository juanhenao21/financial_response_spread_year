...
def taq_save_data(function_name, data, ticker_i, ticker_j, year, month, day):
    ...
    taq_data_analysis_responses_physical_short_long module in pickle files.
    ...
    if (not os.path.isdir(f'../../taq_data/responses_physical_short_long_data'
                          + f'_{year}/{function_name}/')):

        try:
            os.mkdir(f'../../taq_data/responses_physical_short_long_data'
                     + f'_{year}/{function_name}/')
    ...
    if (ticker_i != ticker_j):
        ...
        pickle.dump(data, open(f'../../taq_data/responses_physical_short_long'
                    + f'_data_{year}/{function_name}/{function_name}_{year}'
                    + f'{month}{day}_{ticker_i}i_{ticker_j}j.pickle', 'wb'))
...
def taq_save_plot(function_name, figure, ticker_i, ticker_j, year, month):
    ...
    taq_data_plot_responses_physical_short_long module in png files.
    ...
    if (not os.path.isdir(f'../../taq_plot/responses_physical_short_long_plot'
                          + f'_{year}/{function_name}/')):

        try:
            os.mkdir(f'../../taq_plot/responses_physical_short_long_plot'
                     + f'_{year}/{function_name}/')
            print('Folder to save data created')
    ...
    if (ticker_i != ticker_j):
        ...
        figure.savefig(f'../../taq_plot/responses_physical_short_long_plot'
                       + f'_{year}/{function_name}/{function_name}'
                       + f'_{year}{month}_{ticker_i}i_{ticker_j}j.png')
    ...
    # Self-response
        ...
        figure.savefig(f'../../taq_plot/responses_physical_short_long_plot'
                       + f'_{year}/{function_name}/{function_name}'
                       + f'_{year}{month}_{ticker_i}i.png')
...
def taq_start_folders(year):
    ...
    try:
        os.mkdir(f'../../taq_data/responses_physical_short_long_data_{year}')
        os.mkdir(f'../../taq_plot/responses_physical_short_long_plot_{year}')
        print('Folder to save data created')
...
def taq_initial_data():
    ...
    print()
    print('###############################################')
    print('Response Functions Physical Short Long Analysis')
    print('###############################################')
    print('AG Guhr')
    ...
    print('How many short time lag values do you want to use?')
    n_tau_p = int(input())
    taus_p = []

    for _ in range(n_tau_p):
    ...
    print()
    print('Please enter the time lag value you want to use (i.e. 1000): ')
    tau = int(input())
    ...
    return (year, tickers, tau, taus_p)
