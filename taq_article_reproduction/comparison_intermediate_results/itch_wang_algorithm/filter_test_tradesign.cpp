...
int main(int argc, const char *argv[])
{
    ...
    vector<double>sign_persecond_e1,sign_persecond_e2,sign_persecond_t;
    vector<int>time_second;
    for(int j=0; j<57000000/1000-34800000/1000; j++){
        double sum_sign_e1=0;
        double sum_sign_e2=0;
        double sum_sign_t=0;
        for(int i=0; i<index_EFT.size(); i++) {
            if(sign_e[i]!=2 && time_currentorder[i]/1000>=34800+j && time_currentorder[i]/1000<34801+j){
                sum_sign_e1+=sign_e[i];
                sum_sign_e2+=sign_e[i]*volume_transaction[i];
                sum_sign_t+=sign_t[i];
            }
        }
        sign_persecond_e1.push_back(sgn(sum_sign_e1));
        sign_persecond_e2.push_back(sgn(sum_sign_e2));
        sign_persecond_t.push_back(sgn(sum_sign_t));
        if(sign_persecond_e1[j]==0&&sign_persecond_e2[j]==0&&sign_persecond_t[j]==0){

        }else{
            if(sign_persecond_e1[j]-sign_persecond_t[j]==0)
                same_persecond1++;
            if(sign_persecond_e2[j]-sign_persecond_t[j]==0)
                same_persecond2++;
            total_persecond++;
        }
        if(sign_persecond_e1[j]==0)
            num_sign0_1++;
        if(sign_persecond_e2[j]==0)
            num_sign0_2++;
    ...
    for(int j=0; j<57000000/1000-34800000/1000; j++){
            cout<<time_second[j] <<"   "<<time_format(time_second[j])<<"   "<<sign_persecond_e1[j]<<"   "<<sign_persecond_e2[j]<<"   "<<sign_persecond_t[j]<<endl;
    }
    cout.close();
