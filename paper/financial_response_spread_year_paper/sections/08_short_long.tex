\section{Time lag analysis}\label{sec:short_long}
...
This approach is similar to the concept used in \cite{Wang_2018_b}, where the
price impact for a single trade is estimated by the immediate response on an
event time scale. In our case, we check all over the range of the time lag in
the price response function on a physical time scale.
...
where the superscript $sl$ refers to short-long and the second term of the
