...
the end, the individual returns sum up to a net return, which can be positive
or negative, depending of the type of midpoint price values in the interval.
Thus, in this case, we are assuming in the end that all the returns were
positive or negative, what probably was not the case, and in consequence will
increase or decrease the real value of the net return.
...
becomes negative beyond $\tau \approx 5000$. However, in some cases the maximum
is not observed and rather $R\left(\tau\right)$ keeps increasing mildly
\cite{subtle_nature}.
