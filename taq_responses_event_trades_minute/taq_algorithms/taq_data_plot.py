...
def taq_cross_response_year_avg_responses_event_trades_minute_plot(ticker_i,
    ...
    try:
        ...
        plt.legend(loc='best', fontsize=25)
        plt.title('Cross-response event {} - {}'.format(ticker_i, ticker_j),
                  fontsize=40)
        plt.xlabel(r'Trades per minute', fontsize=35)
        plt.ylabel(''.join((r'$\left\langle r_{i}\left(t-1,\tau\right)\cdot'
...
def main():
    ...
    taq_self_response_year_responses_event_trades_minute_plot(ticker, year, tau)
    taq_cross_response_year_responses_event_trades_minute_plot(ticker_i,
                                                                   ticker_j,
                                                                   year, tau)
