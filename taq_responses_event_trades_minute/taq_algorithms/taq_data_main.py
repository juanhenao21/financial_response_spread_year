...
def main():
    ...
    tickers = ['AAPL', 'MSFT']
    year = '2008'
    tau = [5, 10, 50, 100, 500, 1000]

    with mp.Pool(processes=mp.cpu_count()) as pool:
        pool.starmap(taq_data_analysis.taq_self_response_year_responses_event_trades_minute_data,
                     product(tickers, [year], tau))
        pool.starmap(taq_data_analysis.taq_cross_response_year_responses_event_trades_minute_data,
                      product(tickers, tickers, [year], tau))
